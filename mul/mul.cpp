...
{
    long i,tam;
    for(i=1;i<=a.so[0]/2;i++)
    {
        tam=a.so[i];
        a.so[i]=a.so[a.so[0]-i+1];
        a.so[a.so[0]-i+1]=tam;
    }
}
void daomangb()
{
    long i,tam;
    for(i=1;i<=b.so[0]/2;i++)
    {
        tam=b.so[i];
        b.so[i]=b.so[b.so[0]-i+1];
        b.so[b.so[0]-i+1]=tam;
    }
}
mang congmang(mang a, mang b)
{
    long i,nho;
    mang c;
    c.so[0]=max(a.so[0],b.so[0]);
    nho=0;
    for(i=1;i<=c.so[0];i++)
    {
        c.so[i]=a.so[i]+b.so[i]+nho;
        if(c.so[i]>9)
        {
            c.so[i]=c.so[i]%10;
            nho=1;
